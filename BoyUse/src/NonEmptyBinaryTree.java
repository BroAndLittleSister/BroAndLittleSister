...
public class NonEmptyBinaryTree <T extends Comparable<T>> extends BinaryTree<T> {
	...
	public BinaryTree<T> insert(T d) {
		...
		BinaryTree<T> newOne=new NonEmptyBinaryTree<T>(d);
		if (data==null){
			return newOne;
		}

		if (data.compareTo(d)>0){
			newOne=left.insert(d);
			return new NonEmptyBinaryTree<T>(data,newOne,right);
		}
		else if (data.compareTo(d)==0){
			newOne=new NonEmptyBinaryTree<T>(data,left,right);
			return newOne;
		}
		else {
			newOne=right.insert(d);
			return new NonEmptyBinaryTree<T>(data,left,newOne);
		}
	...
	public BinaryTree<T> delete(T d) {
		// TODO: Add your implementation here
		// ########## YOUR CODE STARTS HERE ##########
		BinaryTree<T> newOne=new NonEmptyBinaryTree<T>(data,left,right);
		BinaryTree<T> newTwo=new NonEmptyBinaryTree<T>(data,left,right);

		if (data.compareTo(d)!=0){

				newOne = left.delete(d);
				newTwo = right.delete(d);

			return new NonEmptyBinaryTree<T>(data,newOne,newTwo);
		}
		else {
			if (left.preOrderShow().equals("")&&right.preOrderShow().equals(""))
				return new EmptyBinaryTree<>();
			else if (left.preOrderShow().equals("")&&!right.preOrderShow().equals("")){
				return right;
			}
			else if (right.preOrderShow().equals("")&&!left.preOrderShow().equals(""))
				return left;
			else{
				T successor=right.smallest();
				data=successor;
				newOne=right.delete(successor);
				return new NonEmptyBinaryTree<T>(data,left,newOne);
	...
	//find successor for x and replace them ,delete the successor in subtree
